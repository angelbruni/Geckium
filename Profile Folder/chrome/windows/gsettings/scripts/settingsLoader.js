			...
			selector.querySelectorAll(".list .item").forEach(item => {
				item.addEventListener("click", () => {
					if (Number.isInteger(parseInt(item.getAttribute("value"))))
						UC_API.Prefs.set(selector.dataset.pref, parseInt(item.getAttribute("value")));
					else
						UC_API.Prefs.set(selector.dataset.pref, `${item.getAttribute("value")}`);
	...
	setTimeout(() => {
		document.querySelectorAll('input[type="text"][data-pref]').forEach(input => {
			...
			input.addEventListener("input", () => {
				UC_API.Prefs.set(input.dataset.pref, input.value);
			})
		})
	}, 10);
}
